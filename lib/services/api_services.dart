...
getHourlyWeather(lat, long) async {
  var link = "https://api.openweathermap.org/data/2.5/forecast?lat=$Latitude&lon=$Longitude&appid=$apiKey&units=metric";
  var res = await http.get(Uri.parse(link));
  if (res.statusCode == 200) {
...
getForecast(lat, long,  value) async {
  var link = "https://api.openweathermap.org/data/2.5/forecast?lat=$Latitude&lon=$Longitude&cnt=$cnt&appid=$apiKey&units=metric";
  var res = await http.get(Uri.parse(link));
  if (res.statusCode == 200) {
    var data = forecastFromJson(res.body.toString());
